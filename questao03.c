...
int main()
{
    ...
    case 1:
        ...
        {
            aluno alunos;
            MPI_Recv(&alunos.nota[1],1,MPI_INT,0,1,MPI_COMM_WORLD, &status);
            alunos.nota[id_processo] = 1;
            MPI_Send(&alunos.nota[1],1,MPI_INT,0,1,MPI_COMM_WORLD);
        }
        break;
    case 2:
        while (1)
        {
            aluno alunos;
            MPI_Recv(&alunos.nota[2],1,MPI_INT,0,1,MPI_COMM_WORLD, &status);
            alunos.nota[id_processo] = 2;
            MPI_Send(&alunos.nota[2],1,MPI_INT,0,1,MPI_COMM_WORLD);
        }
        break;
    case 3:
        while (1)
        {
            aluno alunos;
            MPI_Recv(&alunos.nota[3],1,MPI_INT,0,1,MPI_COMM_WORLD, &status);
            alunos.nota[id_processo] = 3;
            MPI_Send(&alunos.nota[3],1,MPI_INT,0,1,MPI_COMM_WORLD);
        }
        break;
    }
